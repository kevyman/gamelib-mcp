"""Schema migration chain — every step that carries a database to SCHEMA_VERSION.

Split out of the package façade unchanged, purely for size: the ladder was 82 %
of ``__init__.py`` and none of it is reachable outside ``migrate_db``. What
...
``MigrationResult``, the connection helpers, ``init_db``/``migrate_db`` — and
re-exports ``_run_migrations`` so first-touch initialization still reaches it as
``gamelib_mcp.data.db._run_migrations``. Adding a schema version is unchanged:
a new ``_V{N}_SCHEMA_DDL`` in ``schema.py``, a step function here, its
``_MIGRATION_STEPS`` entry, and ``SCHEMA_VERSION`` bumped on the façade.
"""
