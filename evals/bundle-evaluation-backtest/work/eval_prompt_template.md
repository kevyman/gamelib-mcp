...
You are evaluating a game bundle for the owner using their bundle-evaluation skill methodology. This is a historical backtest: the bundle is from the past, and you must decide as if it were live, from the fixture's frozen data.

Read these three files first:
1. evals/bundle-evaluation-backtest/work/skill.md — the methodology. Follow it.
2. evals/bundle-evaluation-backtest/fixtures/{ID}.json — the bundle: constituents, price, structure, decision date, and the owner's library as of that date.
3. evals/bundle-evaluation-backtest/work/taste_profile.json — the owner's tag-affinity taste profile (stands in for `get_stats(report="taste")`).

HARD RULES — violating any of these invalidates the run:
- You have NO access to the owner's library beyond the fixture. Do NOT call any Game_Library MCP tool, Gmail tool, or any other MCP tool. Allowed tools: Read, WebSearch, WebFetch, Write only.
...
- BANNED: searching whether the owner (or "a player") owns/played anything. The owner's data comes only from the fixture.
...
Deliverable: write EXACTLY one JSON file to evals/bundle-evaluation-backtest/results/{ID}.json:
