...
- `admin.py`: `refresh_library` (full or per-platform sync), `detect_farmed_games`, `detect_collapsed_games` (read-only; surfaces games over-merged by name — one platform row holding multiple distinct same-type store IDs), `set_nintendo_session` (VGCS ownership cookies), `set_nintendo_pctl_session` (Parental Controls playtime token)
