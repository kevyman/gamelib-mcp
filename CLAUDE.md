...
- `completion.py`: `suggest_completion_status` (read-only heuristic surfacing completed/abandoned candidates among un-classified, non-farmed, primary-library games — never writes; confirm via `update_game`)
...
- **Completion status**: `games.completion_status` (`playing`/`completed`/`abandoned`, NULL = unset) is set only through `update_game` — no sync or enrichment writer ever touches it. `PLAY_STATE_SQL` treats an explicit `completed` as `played` even when playtime is unknown (e.g. a GOG game with no playtime tracking), but does *not* force `abandoned` to `unplayed`/`played` — that stays purely playtime-derived. Backlog/discovery instead exclude `completed`/`abandoned` via their own filters: `get_library_stats(filter="playing"|"completed"|"abandoned")`, `get_backlog_stats`'s `backlog_hours_hltb`/best-unplayed queries, and `discover_games` (unconditionally, regardless of `unplayed_only`). `suggest_completion_status` is a read-only heuristic (never writes) that ranks un-classified, non-farmed, primary-library games by playtime-vs-HowLongToBeat ratio and last-played staleness; a human confirms a suggestion via `update_game`.
