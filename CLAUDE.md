...
- Optional: `PORT` (default 8000); `LOG_LEVEL` (DEBUG/INFO/WARNING/ERROR, default INFO — DEBUG surfaces per-item enrichment failures); `LIBRARY_REFRESH_INTERVAL_HOURS` (default 24; ≤0 disables the periodic loop, and with it deal alerts); `DEKUDEALS_WISHLIST_URL` (switch2 wishlist source; Nintendo has no wishlist API); `ITAD_API_KEY`/`ITAD_COUNTRY` (Steam/GOG/Epic prices for `get_wishlist(with_prices=True)`; without a key those land in `unpriced`); `DEAL_ALERT_WEBHOOK_URL` (Discord/Slack incoming webhook — wishlist deal alerts at the end of each library refresh, so every `LIBRARY_REFRESH_INTERVAL_HOURS`; empty disables); `SCRAPE_HEAL_REQUIRE_APPROVAL=1` (validated overrides land `pending`, not active).
...
- `deal_alerts.py` — the wishlist deal notifier (`DEAL_ALERT_WEBHOOK_URL`), called at the end of `lifecycle._run_startup_refresh` and never able to fail it; it has no scheduler of its own, so its cadence IS the refresh cadence (`LIBRARY_REFRESH_INTERVAL_HOURS`, default 24h). → patterns/ownership-and-wishlist.md
