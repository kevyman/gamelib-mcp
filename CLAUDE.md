...
- Optional cookie-file paths for `import_purchases` sessions: `NINTENDO_COOKIES_FILE` (the shared accounts.nintendo.com login session — drives both Switch ownership and eShop purchases; set via `set_nintendo_session`), `NINTENDO_EC_COOKIES_FILE` (legacy ~1h eShop session-token fallback), `HUMBLE_COOKIES_FILE`, `STEAM_STORE_COOKIES_FILE` — populated by the matching `set_*_session` tools; defaults live under `data/`.
...
- `library.py`: search_games, search_games_batch (nested-content fallback when nothing matches), get_library_stats (content filter: games/addons/all; always-on addons summary block)
...
- `admin.py`: refresh_library, sync_wishlist, detect_farmed_games, detect_collapsed_games (read-only, within-platform over-merges), detect_cross_platform_collapses (via IGDB external_games), detect_misclassified_dlc (read-only; surfaces candidates for update_game repair loop), split_game (inverse of merge_games), set_nintendo_session, set_nintendo_pctl_session, plus the cookie-session setters sharing `_save_session_cookies`: set_nintendo_ec_session (legacy ~1h eShop fallback), set_humble_session, set_steam_store_session (set_nintendo_session's accounts session is the preferred eShop auth)
- `platforms.py`: get_platform_breakdown (games vs addons split, plus owned_addons per platform), get_wishlist (content_type labels each item), set_hardware_preference, add_game_to_platform (`owned=False` = manual wishlist entry — the only path for PSN), update_game (manual edits recorded in `games.manual_overrides`; parent_game_id/parent_name repair nested rows)
- `acquisition.py`: set_acquisition, set_acquisitions_batch (fill-only by default — re-imports never clobber manual edits; edition/upgrade-pack suffixes stripped via `normalize_purchase_title`; nested content_type matches exact name only, no fuzzy), split_bundle_acquisition (records a multi-game bundle across constituents — even cent-preserving price split, `create_missing` mints new owned rows, `fuzzy=False` prevents collapsing a sequel), get_spending_stats (per-currency, never summed; by_family rolls each base game + its owned DLC/addons), import_purchases (orchestrates `data/purchases/` fetchers; per-source failures don't block others; multi-game bundles diverted to `bundles_needing_split` for split_bundle_acquisition; nested purchase items stay exact-match-only to prevent spend corruption; `create_missing` defaults **True** for minting new owned games from purchases, reported in `created`/`created_details`; `set_acquisitions_batch`'s `create_missing` defaults False)
