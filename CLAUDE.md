...
- Optional: `PORT` (default 8000); `LOG_LEVEL` (DEBUG/INFO/WARNING/ERROR, default INFO — DEBUG surfaces per-item enrichment failures); `LIBRARY_REFRESH_INTERVAL_HOURS` (default 24; ≤0 disables the periodic loop, and with it deal alerts); `DEKUDEALS_WISHLIST_URL` (switch2 wishlist source; Nintendo has no wishlist API); `ITAD_API_KEY`/`ITAD_COUNTRY` (Steam/GOG/Epic prices for `get_wishlist(with_prices=True)`; without a key those land in `unpriced`); `DEAL_ALERT_WEBHOOK_URL` (Discord/Slack incoming webhook — wishlist deal alerts once each library refresh's sync settles, so every `LIBRARY_REFRESH_INTERVAL_HOURS`; empty disables); `SCRAPE_HEAL_REQUIRE_APPROVAL=1` (validated overrides land `pending`, not active).
...
- `deal_alerts.py` — the wishlist deal notifier (`DEAL_ALERT_WEBHOOK_URL`), called by `lifecycle._run_startup_refresh` once the sync result has settled and BEFORE the background-enrichment drain (which can outlive the sync by hours), and never able to fail it; it has no scheduler of its own, so its cadence IS the refresh cadence (`LIBRARY_REFRESH_INTERVAL_HOURS`, default 24h). → patterns/ownership-and-wishlist.md
...
- **IGDB linking order**: `backfill_missing_games` resolves via `external_games` (filtered on `external_game_source`, never the removed `category`; store id → game: the Steam appid, then the GOG product id for a row Steam did not map) first — a batch that maps nothing AND loses its canary uid (The Witcher 3) is an outage and leaves every row retryable — then the stored igdb_id, then name (exact-name lookups are case-insensitive, `name ~`) — and overrides a STORED link only when the mapping's record and the library row agree on the name (`_igdb_name_agrees`), since the mapping is not infallible. A manual `igdb_id` outranks everything. Every title comparison goes through the one `match_key`, and the name gate also reads IGDB's `alternative_names` (the only place "GTA V" is written down); bump `IGDB_RESOLVER_VERSION` when a matching rule changes and stale no-matches requeue themselves (no migration). The drift audit replays the resolver's own acceptance rules (`resolver_would_accept`) before calling a link wrong, so it never contradicts the resolver that made it. → patterns/enrichment-and-igdb.md
- **Game identity (anti-collapse)**: name is only a *cross-platform* reconciliation key, never within-platform. Syncs resolve by store identifier first; the name/fuzzy fallback refuses a row already owning that platform and rejects release-year conflicts. Every sync adopts an identifier-less same-name row on its own platform before minting (`adopt_platform_identifier`; the Steam bulk path's pass 1c) — a purchase import mints identifier-less rows and the sync must claim them, not fork twins. GOG product ids come from the account listing (`gog_session.py`); the lgogdownloader slug list is only the fallback. `split_game` repairs over-merges by hand — there is deliberately no automatic split. → patterns/identity-and-nesting.md
...
- **DLC & nested content**: `is_primary_library_item` is always DERIVED from `content_type ∈ PRIMARY_CONTENT_TYPES`, never set independently. Both writers (`apply_content_classification`, `_apply_igdb_metadata`, kept in sync) enforce the guards: **a parent must stay primary, both ways**, the substance guard, the edition-ownership guard, primary rows keep no parent. Epic DLC (`mainGameItem`) resolves by its own artifact only and mints nested under its base item — never through name/IGDB resolution — and Unreal marketplace assets and mods are skipped; an IGDB `bundle` with no parent is a purchasable compilation and stays primary. → adr/0002, patterns/identity-and-nesting.md
...
- **Assessment recording** (ADR 0006 decision 5): `record_assessment` stores the COMPONENTS of a verdict, not a score; `skill`/`skill_version`/`model` are **declared-only claims, NULL = unknown**, never server-stamped. The Steam appid of an unowned candidate rides on the assessment row (no `game_platforms` row exists to hang an identifier on), and every Steam-appid reader resolves through the one chain — platform identifier → `game_wishlist.store_identifier` → newest assessment — with both fallbacks skipped for a game that already has an identifier row. **Write paths never resolve a name loosely** — exact-or-mint like `add_game_to_platform`; fuzzy matching lives only on the read path. **Hard constraint: verdicts never feed `tag_affinity` or `discover_games`**, and recording never writes the wishlist. → adr/0006, patterns/assessments.md
...
- **Wishlist tracking**: a separate table because a wishlist item may be owned nowhere — never overload `game_platforms.owned=0`. `delete_stale_wishlist_entries(platform, source, keep_game_ids)` never touches other-source rows and runs **only** after every fetched item resolved this round, so a partial fetch is never read as "wishlist is now empty"; the games rows it leaves unreferenced are garbage-collected in the same pass (`delete_unreferenced_games`). A manual write's `source` is `manual` or `assessment` only. → patterns/ownership-and-wishlist.md
