...
- `acquisition.py`: set_acquisition, set_acquisitions_batch (fill-only by default — re-imports never clobber manual edits; edition/upgrade-pack suffixes stripped before name matching via `normalize_purchase_title`), split_bundle_acquisition (records a multi-game bundle across its constituents — even cent-preserving price split, `create_missing` mints new owned rows, `fuzzy=False` so a sequel isn't collapsed onto its predecessor), get_spending_stats (per-currency, never summed across currencies), import_purchases (orchestrates `data/purchases/` fetchers; per-source failures don't block other sources; multi-game bundles — `PurchaseRecord.is_bundle` — are diverted to a `bundles_needing_split` bucket for split_bundle_acquisition instead of the single-game matcher; `create_missing` defaults **True** here — a single-game purchase that matches nothing (identifier → stripped name → fuzzy all miss) is minted as an owned game, since a purchase is a stronger ownership signal than inferred playtime, reported in `created`/`created_details` and previewed via `dry_run`'s `would_create`. `set_acquisitions_batch`'s `create_missing` defaults False)
