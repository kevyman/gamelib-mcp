...
2. Cross each unowned game's tags and review signal (already in the Step 1 results: `steam_review_desc`, `metacritic_score`, `opencritic_score`, `hltb_main`) against the profile and the standing priors (single-player story-rich indie ↑, roguelite deckbuilders proven, multiplayer-only/live-service hard penalty).
3. Sort every constituent into three want-tiers:
...
- **His own track record as evidence**: `get_stats(report="spending")` → `by_bundle` shows how past bundles actually turned out (bought vs. played), and `report="backlog"` → `unplayed_spend` grounds "does he need 8 more games". Descriptive evidence for the verdict, never a score — and per-game cost-per-hour stays banned exactly as in game-quality.
